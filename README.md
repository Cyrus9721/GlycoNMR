...
Current prediction and data categories relative to important next steps. In our manuscript, we have just predicted constant solvent (D2O, water) solution-state NMR spectral peaks from 3D structure files. This is the most common forms of publicly available data. Future work should seek to integrate other solvents, temperature (which is often missing in the meta data of current carbohydrate databases), full spectra files (which are often omitted, just providing tables of peak values), 2D NMR data, and possibly multimodal data such as mass spectroscopy + NMR. Experimental researchers in carbohydrate structure should continue to improve their standards of complete meta-data and spectra reporting, as well as upload sets of possible structure files that could be consistent with a given spectra rather than just a single structure file of their best guess (when warranted).
